...
namespace App\Controllers\Report\Pr;
...
class Index extends Controller
{
	...
	public function index()
	{
		...
		foreach ($data["data"] as $key => $value) {
			...
			if ($value->pr_part_flag == 1  || in_array($value->part_id, $dataPerubahanStatus["data"])) {
				...
				$value->notes = preg_replace('/\bREQUIREMENT\s*:\s*/i', '', (string) $value->notes);
				$value->specification = "\n" . $value->specification;
	...
	public function dataPerubahan($id)
	{
		try {
			$session = session();
			$model = new PrModel();
			$data = $model->db->query("select * from (SELECT row_number() over(PARTITION by b.id, c.id ORDER by b.idx, c.idx desc) as rn, a.*, d.item_no, b.id as part_id, b.notes, c.qty, c.description, d.item_name, d.original_manufacture, d.manufacture_pn, d.specification, d.unit, d.item_type as mpitemtype, d.article_no,
			...
			WHERE a.id = $id)s where rn = 1 order by order_no, cpr_part_id, cid;");
			if (!$data)
				return ["status" => false, "data" => $model->db->error()];
			$data = $data->getResult();
	...
	public function dataPerubahanStatus($id)
	{
		try {
			...
			WHERE a.id = $id)s order by order_no, cpr_part_id, cid;");
			if (!$data)
				return ["status" => false, "data" => $model->db->error()];
			$data = $data->getResult();
