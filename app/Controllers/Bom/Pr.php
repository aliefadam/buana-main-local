...
// 		return $q;
		...
		if(isset($s["userid"])){
			...
			insert into pr_part_history (
				`id`, `flag`, `item_id`, `supplier_id`, `quotation_no`, `quotation_date`, `notes`, `pr_id`, `created_date`, `created_by`, `modified_date`, `modified_by`
			)
			(SELECT `id`, `flag`, `item_id`, `supplier_id`, `quotation_no`, `quotation_date`, `notes`, (select idx from pr_history where id = ".$json['id']." order by idx desc limit 1) as `pr_id`, `created_date`, `created_by`, `modified_date`, `modified_by` FROM `pr_part` where pr_id = ".$json['id'].")
			");
			$model->db->query("
			...
			(
				...
				FROM `pr_subledger` s where pr_part_id in
