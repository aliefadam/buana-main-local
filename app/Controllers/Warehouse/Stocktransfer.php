...
use App\Models\Warehouse\StocktransferModel;
 
class Stocktransfer extends ResourceController
{
    use ResponseTrait;
	...
	public function insertInfo($info, $uid, $pk, $approval){
		$model = new StocktransferModel();
		$data = $model->db->query("insert into info (info, uid, url) values('".$info."', '".$uid."', 'https://internal.buanamultiteknik.com/api/transaction/stocktransfer/info?approval=".$approval."&id=".$pk."')");
	}
	...
	public function revisi(){
        ...
        $json = $_REQUEST;
		$q = $model->query("select s.*,
		LPAD(s.id, 4, 0) as stocktransfer_no
		...
		if($d->approved<4 && isset($s["userid"])){
			$revBy = $s["userid"];
			$model->db->query("
			insert into stocktransfer_history (
			...
			(SELECT `id`, $revBy, `po_id`, `approved_by`, `approved_date`, `approved2_by`, `approved2_date`, `approved3_by`, `approved3_date`, `rejected_date`, `rejected_by`, `created_date`, `created_by`, `modified_date`, `modified_by`, `flag`, `approved`, `peminta_id`, `notes`, `kabag_id`, `ask_approval_notes`, `approval_notes`, `approval2_notes`, `approval3_notes`, `photo`, `rev` FROM `stocktransfer` where id = ".$json['id'].")
			");

			$model->db->query("
			insert into stock_history (
				`id`, `item_id`, `qty`, `allocation`, `po_id`, `created_date`, `created_by`, `modified_date`, `modified_by`, `purchase_order_item_id`, `flag`, `stocktransfer_id`, `notes`, `photo`
			)
			(SELECT `id`, `item_id`, `qty`, `allocation`, `po_id`, `created_date`, `created_by`, `modified_date`, `modified_by`, `purchase_order_item_id`, `flag`, (select idx from stocktransfer_history where id = ".$json['id']." order by id desc limit 1) as `stocktransfer_id`, `notes`, `photo` FROM `stock` where stocktransfer_id = ".$json['id'].")
			");
			$rev = number_format($d->rev, 0)+1;
			$model->db->query("update stocktransfer set 
			approved_by = null, 
			approved_date = null, 
			approved2_by = null, 
			approved2_date = null, 
			...
			$msg = view('email/stocktransfer_revisi',[
				'stocktransfer_no' => $d->stocktransfer_no,
				'notes' => $d->notes
			]);
			helper(['Wa_helper']);
			$msgWa = preg_replace([
    ...
    public function update($pk = null)
    {
		...
		if(isset($data["approved"])){
			if($data["approved"]==2){
				...
				$this->insertInfo("<b>Approved by Peminta:</b> ".date("Y-m-d H:i:s")."", "stocktransfer_approved_".$pk, $pk, 1);
			}
			if($data["approved"]==3){
				$data["approved2_by"] = $s["userid"];
				$data["approved2_date"] =  date("Y-m-d H:i:s");
				$this->insertInfo("<b>Approved by Kepala Logistik:</b> ".date("Y-m-d H:i:s")."", "stocktransfer_approved2_".$pk, $pk, 2);
			}
			if($data["approved"]==4){
				$data["approved3_by"] = $s["userid"];
				$data["approved3_date"] =  date("Y-m-d H:i:s");
				$this->insertInfo("<b>Approved by Kabag:</b> ".date("Y-m-d H:i:s")."", "stocktransfer_approved3_".$pk, $pk, 3);
			}
			if($data["approved"]==-1){
			...
			/*$msg = view('email/stocktransfer_approved',[
		...
		if(isset($data["approved"])){
			if($data["approved"]==4)
				$model->query("update stock set flag = 1 where stocktransfer_id = ".$pk);
		}
		$model->transComplete();
