...
protected function initialize()
{
    ...
    function read($json){
		...
		$totalQuery = $db->query("select count(*) as total from (select ".join(',', array_map(array($this, 'addPrefix'), $this->allowedFields)).", i.item_no, i.item_name from $q) s $where");
		$totalQuery = $totalQuery->getResult();
        $res = $query->getResult();
        if(count($res) > 0){
            $id = $res[0]->purchase_order_id;
            $query = $db->query("
                select sum(order_qty * price_per_item)+a.charge1+a.charge2 as grand_total from purchase_order_item s left join purchase_order a on a.id = s.purchase_order_id where s.purchase_order_id = $id and s.flag=1
            ");
    ...
    function readItem($po_id){
        helper(['Query_helper']);
        
        $db = db_connect();
		$where = '';
        if(trim($where)=='')
            $where = 'where s.flag = 1 and purchase_order_id = '.$po_id;
        else
            $where .= ' and s.flag = 1 and purchase_order_id = '.$po_id;
		$q = "`$this->table` s 
		left join bom_header h on h.id = s.bom_id 
		left join m_item i on i.id = s.item_id 
		left join purchase_order p on p.id = s.purchase_order_id 
		...
		left join pr r on r.id = t.pr_id";
        $query = $db->query("select 
        (select sum(order_qty * price_per_item) from purchase_order_item where purchase_order_id = s.purchase_order_id and s.flag=1) as total_item_price , 
        s.* from
        (select ".join(',', array_map(array($this, 'addPrefix'), $this->allowedFields)).", h.bom_no, i.item_name, i.item_no, i.unit, i.original_manufacture, i.manufacture_pn, i.specification, i.article_no, p.currency, (s.order_qty*s.price_per_item) as total_price_per_item, coalesce(r.pr_no, h.bom_no) as bom_pr from $q) s 
		$where ");
