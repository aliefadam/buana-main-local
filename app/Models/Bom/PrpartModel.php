...
class PrpartModel extends Model
{
    ...
    function read($json){
        ...
        if(trim($where)=='')
            ...
            $where .= ' and s.flag = 1';
		//case when coalesce(po.flag, 0) = 1 and coalesce(poi.flag, 0) = 1 then 1 else 0 end as in_po
			// left join purchase_order_item poi on poi.pr_part_id = s.id
			// left join purchase_order po on po.id = poi.purchase_order_id
		$q = "select *, total_pr_qty - total_order as left_qty from (select Row_number() over(partition by s.id order by ps.id) as rn,
			".join(',', array_map(array($this, 'addPrefix'), $this->allowedFields)).", 
			(
                select count(pi.id) from purchase_order_item pi 
				left join purchase_order po on po.id=pi.purchase_order_id where pi.pr_part_id = s.id and ps.id=pi.subledger_id and po.approved>=0 and pi.flag=1
            ) as in_po, 
    		coalesce(
                (
    		...
    		(
            	select sum(qty) from pr_subledger where pr_part_id = s.id and flag = 1
            ) as total_pr_qty,
			i.item_no, i.item_name, i.original_manufacture, concat(i.item_no, '-', i.item_name) as itemfull,
			...
			left join users m on m.id = s.modified_by)s
			";
