...
class PurchaseOrderItemGroupHistoryModel extends Model
{
    protected $table = 'purchase_order_item_history';
    protected $returnType     = 'object';
    protected $primaryKey = 'id';
    protected $allowedFields = ["id",
    "purchase_order_id",
    "bom_id",
    "price_per_item", "order_qty", "item_id", "active", 'flag', 'pr_part_id', "allocation", 'created_date', 'created_by', 'modified_date', 'modified_by'
];
...
protected function initialize()
{
    ...
    function read($json){
		...
		SELECT i.datasheet, ms.pic_name, p.currency, s.*, order_qty*price_per_item as total_price_per_item, i.specification from (
			select s.flag, s.purchase_order_id, s.item_id, i.item_no, i.item_name, i.unit, i.original_manufacture, i.manufacture_pn, sum(s.order_qty) as order_qty, 
			(select price_per_item from purchase_order_item_history where purchase_order_id = s.purchase_order_id and item_id = s.item_id limit 1) as price_per_item 
			from purchase_order_item_history s left join m_item i on i.id = s.item_id
		...
		left join purchase_order_history p on p.id = s.purchase_order_id
        ...
        if(count($res) > 0){
            ...
            $query = $db->query("
                select a.charge1, a.charge2, (select sum(total) as total from (select order_qty*price_per_item as total from purchase_order_item_history where purchase_order_id = $id)k) as total from purchase_order_item_history s left join purchase_order a on a.id = s.purchase_order_id where s.purchase_order_id = $id limit 1
            ");
