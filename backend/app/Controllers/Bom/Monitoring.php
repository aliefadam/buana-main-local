...
use App\Models\Bom\MonitoringModel;
    ...
    public function index()
    {
        ...
        try {
            ...
            if($this->request->getMethod() == 'put'){
                ...
                $array = ['CNY' => 0, 'IDR' => 0, 'EUR' => 0, 'GBP' => 0, 'USD' => 0];
                foreach ($data[1] as $value) {
                    if (isset($array[$value->currency])) {
                        $array[$value->currency] += $value->grand_total;
                    }
                }
                return $this->respond(['status' => $data[0], 'data'=>$data[1], 'total' => $data[2], 'sum' => $array], 200);
            }
        } catch (Exception $e) {
