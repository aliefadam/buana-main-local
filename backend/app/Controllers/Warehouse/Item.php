...
use App\Models\Warehouse\ItemModel;
...
class Item extends ResourceController
{
    ...
    public function create()
    {
		...
		if(isset($json->item)){
			$q = $model->db->query("insert into warehouse_item (item_id, rack_id, qty, created_by) values((select id from m_item where item_no = ?), ?, ?, ?)", [$json->item, $json->rack, $json->qty, $s["userid"]]);
			if(!$q){
				$response = [
					'status'   => false,
					'data'    => $model->db->error()
