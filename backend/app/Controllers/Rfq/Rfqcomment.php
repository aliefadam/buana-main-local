...
class Rfqcomment extends ResourceController
{
    use ResponseTrait;
    ...
    public function create()
    {
        ...
        {
			if(isset($json[$value]))//->{$value}))
				$data[$value] = $json[$value];//->{$value};
        }
		$session = session();
		$s = $session->get();
		...
		if(!empty($doc)){
			try {
				if (!$doc->hasMoved()) {
					...
					$filepath = './uploads/rfq'.$json["rfq_id"].'/';
