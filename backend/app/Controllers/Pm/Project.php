...
use App\Models\Pm\ProjectModel;
use App\Models\Pm\ProjecthistModel;
...
class Project extends ResourceController
{
    ...
    public function update($pk = null)
    {
		...
		$affected = $model->affectedRows();
		if($affected != 0){
			$projecthist = new ProjecthistModel();
			$hist = [];
			//$data["$table"] = $model->table;
			$hist["history"] = json_encode($data, true);
			$hist["project_id"] = $pk;
			if(isset($s["userid"]))
				$hist["created_by"] = $s["userid"];
			$projecthist->insert($hist);
			$response = [
				'status'   => true
			];
		}
