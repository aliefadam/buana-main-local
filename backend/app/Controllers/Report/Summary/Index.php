<?php namespace App\Controllers\Report\Summary;
...
class Index extends Controller
{
    ...
    public function index(){
		...
		$total = 0;
		$debugs = array();
        $datereport=$json["dateactivity"];

        $data = $this->data($json["dateactivity"]);
		...
		$vue = file_get_contents('./report/summary_rebuild.vue');
    ...
    public function data($dateactivity){
        try {
			// $date=new \DateTime('yesterday');
			// $date=$date->format('Y-m-d');
            $session = session();
            $model = new ReportItemGroupModel();
            $data = $model->db->query("SELECT ROW_NUMBER() OVER() AS no, ri.parent_id, ri.section_id, ri.partno_id, ri.subassy_id, case when ri.type=1 then 'Replacement' when ri.type=2 then 'Repair' when ri.type=3 then 'Installation' when ri.type=4 then 'Pre-assembly' end as type_report, ri.attachment, i.manufacture_pn, ri.item_replacement_id from report_parent_rebuild s 
            left join report_item_rebuild ri on ri.parent_id = s.id
            left join m_item i on i.id =ri.item_replacement_id 
            where s.date='$dateactivity' and ri.subassy_id is not null and ri.flag=1 and s.flag=1");
			if(!$data)
