...
class BomitemspbModel extends Model
{
    ...
    function read($json){
		...
		$q = "select s.*, (s.qty-received_qty) as remaining_qty, bi.unit_qty, bi.description, bi.material, bi.mass from (
			...
			left join (
				...
				$q = "select s.*, (s.qty-received_qty) as remaining_qty from (
				...
				s.qty as qty, sum(coalesce(t.qty, 0)) as received_qty, br.bom_header_id
				FROM `bom_receive_item` s
