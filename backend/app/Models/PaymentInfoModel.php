...
class PaymentInfoModel extends Model
{
    ...
    function read($json){
        ...
        if(trim($where)=='')
            ...
            $where .= ' and s.flag = 1';
		$q = "`$this->table` s left join invoice i on i.id = s.invoice_id left join m_supplier ms on ms.id = i.supplier_id left join purchase_order p on p.id = i.po_id left join email e on e.message like CONCAT('%', i.invoice_no ,'%')";
        
		$q2 = "
		select ".join(',', array_map(array($this, 'addPrefix'), $this->allowedFields)).", i.invoice_no, invoice_date, i.as_reference, i.uraian, i.payment_pct, i.proof_of_transfer, i.is_paid, i.paid_date,
		ms.name as supplier_name, ms.bank, ms.bank_account_no, ms.currency, ms.bank_account_name, ms.bic_swift_code, p.title as title,
		...
		end as invoice_total_price, p.id as po_id, p.po_no, e.message,
        case when e.message is not null then 1 else 0 end as status
		from $q";
