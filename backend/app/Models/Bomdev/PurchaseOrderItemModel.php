...
class PurchaseOrderItemModel extends Model
{
    ...
    "purchase_order_id",
    "bom_id",
    "price_per_item", "order_qty", "item_id", "active", 'flag', 'allocation'
];
...
protected function initialize()
{
    ...
    function read($json){
        ...
        if(trim($where)=='')
            ...
            $where .= ' and s.flag = 1';
		$q = "`$this->table` s left join bom_header h on h.id = s.bom_id left join m_item i on i.id = s.item_id left join purchase_order p on p.id = s.purchase_order_id $where";
        ...
        s.* from
        (select ".join(',', array_map(array($this, 'addPrefix'), $this->allowedFields)).", h.bom_no, i.item_name, i.item_no, i.unit, i.original_manufacture, i.manufacture_pn, i.specification, p.currency, (s.order_qty*s.price_per_item) as total_price_per_item from $q) s $order limit $offset, $limit");
