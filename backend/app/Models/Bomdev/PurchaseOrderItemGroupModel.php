...
class PurchaseOrderItemGroupModel extends Model
{
    ...
    "purchase_order_id",
    "bom_id",
    "price_per_item", "order_qty", "item_id", "active", 'flag'
];
...
protected function initialize()
{
    ...
    function read($json){
		...
		SELECT i.datasheet, ms.pic_name, p.currency, s.*, order_qty*price_per_item as total_price_per_item, i.specification from (
			...
			(select price_per_item from purchase_order_item where purchase_order_id = s.purchase_order_id and item_id = s.item_id limit 1) as price_per_item 
        ...
        if(count($res) > 0){
            ...
            $query = $db->query("
                select a.charge1, a.charge2, (select sum(total) as total from (select order_qty*price_per_item as total from purchase_order_item where purchase_order_id = $id)k) as total from purchase_order_item s left join purchase_order a on a.id = s.purchase_order_id where s.purchase_order_id = $id limit 1
            ");
