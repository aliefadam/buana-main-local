...
class PurchaseOrderItemHistoryModel extends Model
{
    ...
    "purchase_order_id",
    "bom_id",
    "price_per_item", "order_qty", "item_id", "active", 'flag', 'allocation', 'complete_qty', 'created_date', 'created_by', 'modified_date', 'modified_by'
];
...
protected function initialize()
{
    ...
    function read($json){
        ...
        (select sum(order_qty * price_per_item) from purchase_order_item_history where purchase_order_id = s.purchase_order_id) as total_item_price , 
        s.* from
        ...
        if(count($res) > 0){
            ...
            $query = $db->query("
                select sum(order_qty * price_per_item)+a.charge1+a.charge2 as grand_total from purchase_order_item_history s left join purchase_order_history a on a.id = s.purchase_order_id where s.purchase_order_id = $id
            ");
