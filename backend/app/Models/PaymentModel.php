...
class PaymentModel extends Model
{
	...
	"notes", 'flag', 'approved', 'approved1', 'approved1_date', 'approved2', 'approved2_date', 'done', 'rev', 'created_by', 'created_date', 'modified_by', 'modified_date', 'credit_note','credit_note_id', 'is_request_payment'];
	...
	function transferstock($json, $user_id){
		...
		foreach ($data as $key => $val) {
			...
			$query[] = ["insert into stock(item_id, allocation, qty, po_id, created_by, purchase_order_item_id, spb_id, notes, photo, barcode)  select item_id, '".$json->allocation."', '".$json->qty."', '".$val->po_id."', '$user_id', '".$json->id."', '".$json->spb_id."', '".urldecode($json->notes ?? '')."', '".urldecode($json->photo)."', '".($json->barcode ?? '')."' 
			from (
				SELECT poi.*, po.id as po_id from purchase_order_item poi left join purchase_order po on po.id = poi.purchase_order_id and po.flag = 1 left join bom_header bh on bh.id = poi.bom_id and bh.flag = 1 left join bom_item bi on bi.bom_header_id = bh.id and bi.flag = 1 WHERE poi.flag = 1 and po.is_complete = 0 and po.id = ".$json->po_id." and poi.id = ".$json->id."
			)s where id = ".$json->id." and complete_qty = '".$json->complete_qty."'",
			"update purchase_order_item set complete_qty = (complete_qty + ".$json->qty.") where id = ".$json->id." and complete_qty = '".$json->complete_qty."'"];
	...
	function transferstockBom($json, $user_id){
		...
		$this->db->query("insert into stock(item_id, allocation, qty, po_id, created_by, purchase_order_item_id, spb_id, notes, photo, barcode, bom_receive_id) values ".$param);
		$this->db->transComplete();
		if($this->db->transStatus() === false){
			$this->db->transRollback();
			return [false, $this->db->error()];
