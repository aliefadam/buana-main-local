...
class Data extends Controller
{
	...
	public function reportpo(){
		$json = $_REQUEST;
		//http://103.171.84.71/api/report?type=pdf&file=po&filename=po&engine=easytemplate&po_id=1974&idx=123fwef235324
		$url = "http://103.171.84.71/api/report?type=pdf&file=po&filename=".$json['filename']."&engine=easytemplate&po_id=".$json['id']."&idx=".uniqid();
...
// 			return $this->respond(['status' => false, 'message' => 'You area not allowed to view this page!'], 401);
		...
		$json = $_REQUEST;
		$url = "http://103.171.84.71/api/report?type=pdf&file=po2&filename=".$json['filename']."&engine=easytemplate&po_id=".$json['id']."&idx=".uniqid();
		$file = "./download/".$json['filename'].".pdf";
		//Get content as a string. You can get local content or download it from the web.
		/* $downloadedFile = file_get_contents($url);
		file_put_contents($file, $downloadedFile); */
    ...
    public function reportpo3(){
		$json = $_REQUEST;
		$url = "http://103.171.84.71/api/report?type=pdf&file=po3&filename=".$json['filename']."&engine=easytemplate&po_id=".$json['id']."&idx=".uniqid();
		$file = "./download/".$json['filename'].".pdf";
		//Get content as a string. You can get local content or download it from the web.
		/* $downloadedFile = file_get_contents($url);
		file_put_contents($file, $downloadedFile); */
    ...
    public function reportponoprice(){
		...
		$json = $_REQUEST;
		$url = "http://103.171.84.71/api/report?noprice=true&type=pdf&file=po2&filename=".$json['filename']."&engine=easytemplate&po_id=".$json['id']."&idx=".uniqid();
		$file = "./download/".$json['filename'].".pdf";
		//Get content as a string. You can get local content or download it from the web.
		/* $downloadedFile = file_get_contents($url);
		file_put_contents($file, $downloadedFile); */
