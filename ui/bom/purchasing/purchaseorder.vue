    ...
    v-else>Print</template>
            ...
            <template v-slot:item.grand_total_price="props">
                <div style="white-space: nowrap;">
                    ...
                    <b>Total Item</b> <br />
                    {{ props.item.currency }} {{ Number(props.item.grand_total_price || 0).format(2, 3) }}
                </div>
            </template>
        ...
        <v-action-dialog :actions="false" v-model="dialogRemainingBudget" title="Remaining Budget" min-height="75%"
            ...
            <div style="padding: 12px">
                <table class="default-table with-border" style="width: 100%">
                    <thead>
                        <tr>
                            <th style="padding: 5px; width: 150px" :rowspan="showOperationalRemainingBreakdown ? 2 : 1">
                                Charged To
                            </th>
                            <th style="padding: 5px; width: 150px" :rowspan="showOperationalRemainingBreakdown ? 2 : 1">
                                Item No
                            ...
                            <th style="padding: 5px" :rowspan="showOperationalRemainingBreakdown ? 2 : 1">
                                QTY
                            </th>
                            <th style="padding: 5px" :rowspan="showOperationalRemainingBreakdown ? 2 : 1">
                                Price
                            ...
                            <th style="padding: 5px" v-if="showOperationalRemainingBreakdown" colspan="2">
                                Remaining Budget
                            </th>
                            <th style="padding: 5px" v-else :rowspan="showOperationalRemainingBreakdown ? 2 : 1">
                                Remaining Budget
                            </th>
                            <th style="padding: 5px" :rowspan="showOperationalRemainingBreakdown ? 2 : 1">
                    ...
                    <tbody>
                        <tr v-if="remainingBudgetLoading">
                            <td :colspan="showOperationalRemainingBreakdown ? 12 : 11"
                        ...
                        <tr v-else-if="remainingBudgetItems.length === 0">
                            <td style="padding: 5px; text-align: center;" :colspan="showOperationalRemainingBreakdown ? 12 : 11">
                                No data
                            </td>
                        </tr>
                        <tr v-else v-for="(item, index) in remainingBudgetItems" :key="index">
                            <td style="padding: 5px" v-if="item.show_project_budget" :rowspan="item.project_budget_rowspan">
                            ...
                            <td style="padding: 5px" v-if="item.show_remaining" :rowspan="item.remaining_rowspan" :colspan="
...
.po-title-menu .v-subheader {
    font-weight: 600 !important;
    color: #2b2b2b !important;
}

.po-title-menu .v-list-item__title {
            ...
            try {
                ...
                rows.map(function (row, idx) {
                    ...
                    if (row && row.disabled === true) {
                        ...
                        formatted.push({ header: row.text || '-' })
                        return
                    }
                ...
                self.headersObj.title.data_value = formatted
                self.headers = App.updateObject(self.headers)
                self.defaultForm = JSONfn.parse(JSONfn.stringify(self.headers))
            } catch (err) {
                console.log('Failed loading PO title options', err)
            }
        },
        ...
        calculateRemainingBudget: async function () {
            ...
            try {
                ...
                Object.keys(groupedProjectIds).map((price) => {
                    ...
                    requests.push(
                        axios
                            .get('https://panel.buanamultiteknik.com/api/budget/project-budget/subledger', {
                                params: {
                                    price: price,
                ...
                Object.keys(groupedOperationalIds).map((price) => {
                    ...
                    requests.push(
                        axios
                            .get('https://panel.buanamultiteknik.com/api/budget/operational-budget/subledger', {
                                params: {
                                    price: price,
                            ...
                            .then((budgetRes) => {
                                ...
                                budgetData.map((d) => {
                                    ...
                                    operationalLabelMap[d.subledger_id] =
                                        department + ' ➝ ' + typeOperational + ' ➝ ' + subTypeOperational
                                })
                            }),
                    )
                })
