    ...
    >
      ...
      <template v-slot:menu-after-filter>
        ...
        <v-btn
          ...
          >Validate</v-btn
        >
        <!-- <v-btn small color="warning" outlined v-if="!nointeraction" :disabled="selected == false" @click="action='reject'; dialogNote=true" >Reject</v-btn> -->
      </template>
  ...
  methods: {
    ...
    approve: async function () {
      ...
      if (c) {
        ...
        params["approved_notes"] = self.approval_notes;
      ...
      if (!q) return true;
        ...
        try {
          var r = await axios.get(App.url + "suratjalan/suratjalan/revisi", {
            params: {
              id: self.selected.id,
              revised_notes: self.approval_notes,
            },
          });
          if (!r.data.status) App.errorMsg(r.data);
          else {
    ...
    reject: async function () {
      ...
      if (c) {
        ...
        params["rejected_notes"] = self.approval_notes;
        var r = await axios.put(App.url + "suratjalan/suratjalan", params);
        if (!r.data.status) App.errorMsg(r.data);
        else {
          self.$refs.template.getItems();
