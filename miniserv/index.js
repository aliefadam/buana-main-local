...
module.exports = function (App) {
  ...
  Number.prototype.format = function (n, x, dec, delim) {
    ...
    var dec = dec || ",",
      delim = delim || ".";
    var ret = this.toFixed(Math.max(0, ~~n)).replace(
      new RegExp(re, "g"),
      "$&{,}",
    );
    var num = ret.split(".");
  ...
  numbered.stringify = function (value) {
    ...
    if (interval === 1) {
      return (
        NUMBER_MAP[Math.floor(num / 10) * 10] +
        "-" +
        numbered.stringify(Math.floor(num % 10))
      );
    }
    ...
    if (CARDINAL_MAP[interval]) {
      ...
      sentence.push(
        numbered.stringify(Math.floor(num / Math.pow(10, interval))),
      );
  ...
  return class index extends App.module {
    ...
    async init(opt) {
      ...
      var data = await axios.post(
        "https://" + urlDomain + ".buanamultiteknik.com/api/data/get",
        {
        ...
        20 > a
          ...
          : 100 > a
            ? ((b = parseInt(String(a / 10).substr(0, 1))),
              (b = c[b] + " Puluh " + c[a % 10]))
            : 200 > a
              ? (b = "Seratus " + self.terbilang(a - 100))
              : 1e3 > a
                ? ((b = parseInt(String(a / 100).substr(0, 1))),
                  (b = c[b] + " Ratus " + self.terbilang(a % 100)))
                : 2e3 > a
                  ? (b = "Seribu " + self.terbilang(a - 1e3))
                  : 1e4 > a
                    ? ((b = parseInt(String(a / 1e3).substr(0, 1))),
                      (b = c[b] + " Ribu " + self.terbilang(a % 1e3)))
                    : 1e5 > a
                      ? ((b = parseInt(String(a / 100).substr(0, 2))),
                        (a %= 1e3),
                        (b = self.terbilang(b) + " Ribu " + self.terbilang(a)))
                      : 1e6 > a
                        ? ((b = parseInt(String(a / 1e3).substr(0, 3))),
                          (a %= 1e3),
                          (b =
                            self.terbilang(b) + " Ribu " + self.terbilang(a)))
                        : 1e8 > a
                          ? ((b = parseInt(String(a / 1e6).substr(0, 4))),
                            (a %= 1e6),
                            (b =
                              self.terbilang(b) + " Juta " + self.terbilang(a)))
                          : 1e9 > a
                            ? ((b = parseInt(String(a / 1e6).substr(0, 4))),
                              (a %= 1e6),
                              (b =
    ...
    get() {
      ...
      return new Promise(async (resolve, reject) => {
        try {
          ...
          if (self.$get.fake == 1)
            var taxUsageTotal = await axios.post(
              "https://" + urlDomain + ".buanamultiteknik.com/api/data/get",
              {
                q: `select
								coalesce(sum(case when coalesce(use_ppn, 0) = 1 then coalesce(ppn_value, 0) else 0 end), 0) as ppn_total,
								coalesce(sum(case when coalesce(use_pph, 0) = 1 then coalesce(pph_value, 0) else 0 end), 0) as pph_total
								from fake_purchase_order_item
								where flag = 1 and purchase_order_id = ${po_id}`,
              },
            );
          else
            var taxUsageTotal = await axios.post(
          ...
          data.data.data.map((val, i) => {
            ...
            if (val.promised_delivery_date != null) {
              var promised_delivery_date = val.promised_delivery_date
                .split(" ")[0]
                .split("-")
                .reverse();
              promised_delivery_date[1] =
                bulan[Number(promised_delivery_date[1])];
            ...
            if (val.item_type.trim() == "Jasa") {
              ...
              var original_manufacture1 =
                "Original Manufacture: " + val.original_manufacture;
              var manufacture_pn1 =
          ...
          if (po.data.data[0].approved == 3) {
            var barcode = await self.barcode.get({
              ...
              text:
                "https://" +
                urlDomain +
                ".buanamultiteknik.com/#/sa/info/po_approved_" +
                po_id, //'https://internal.buanamultiteknik.com/#/sa/info?id='+po_id
              showtext: false,
            });
          ...
          if (po.data.data[0].approved_date != null) {
            var approval_2_date = po.data.data[0].approved_date
              .split("-")
              .reverse();
          ...
          items.map(function (val) {
            ...
            name = val.dir;
            val.total_price_per_item = Number(val.total_price_per_item).format(
              2,
              3,
            );
          ...
          var ppnTotalBase = Number(
            (taxUsageTotal.data.data[0] || {}).ppn_total || 0,
          );
          var pphTotalBase = Number(
            (taxUsageTotal.data.data[0] || {}).pph_total || 0,
          );
          total = Number(total || 0) + ppnTotalBase - pphTotalBase;
          var ppn_total = cr + " " + Number(ppnTotalBase).format(2, 3);
          var pph_total = cr + " " + Number(pphTotalBase).format(2, 3);
          var res = {
            status: true,
            data: {
