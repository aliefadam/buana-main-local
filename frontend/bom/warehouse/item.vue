  ...
  >
        ...
        <v-btn
          ...
          @click="
            function () {
              ...
              if (qtyBarcode) {
                if (!isNaN(qtyBarcode)) showBarcode = true;
              }
            }
          "
          :disabled="!selected"
    ...
    >
      ...
      <v-print ref="vprintbarcode" style="color: #000">
        <!-- <item-barcode :qty="qtyBarcode" v-model="dataBarcode" :key="version" :page-break="dataBarcode.length>1 ? true: false" :chunk-size="2"></item-barcode> -->
        <item-barcode
          :qty="qtyBarcode"
          v-model="dataBarcode"
          :key="version"
        ...
        try {
          ...
          self.$refs.template.selectedRowTmp.map((val) => {
            self.dataBarcode.push({
              barcodeQr: "BMT.ITEM." + val.item_no,
