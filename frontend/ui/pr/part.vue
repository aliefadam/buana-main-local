<template>
	<v-container v-observe-visibility="onVisible" class="no-padding-margin" style="padding : 0 !important; height: 100%; display: flex; flex-direction: column; margin: 0; width: 100%; max-width: 100%;">
		<v-template :table-loading.sync="loading" :show-expand="showExpand" :single-expand="singleExpand" :data="data" :items-options="itemsOptions" @update:selected-row="onSelectedRow" @update:selected-row-all="onSelectedRowAll" delete-mode="delete" :active-column="activeColumn" v-model="value" ref="template" :item-key="itemKey" :url="url" :headers="headers" :name="name" :table-only="tableOnly" :table-fixed-header="tableFixedHeader">
            ...
            <template v-slot:append-menu>
			 ...
			 <template v-slot:menu-after-filter>
				<v-btn small color="primary" outlined :disabled="selected == false" @click="dialogSubledger=true" v-if="!isDashboard" >Subledger</v-btn>
				<!-- <v-btn small color="primary" outlined  v-if="tableOnly!=true" @click="dialogBOM=true" >Add From BOM</v-btn> -->
				<!-- <v-btn color="warning" :disabled="!selected" v-if="tableOnly!=true" outlined small>
		...
		<v-action-dialog v-model="dialogBOM" title="Add From BOM" min-height="75%" @save="saveFromBOM">
            <!-- <v-input-number v-model="order_no"></v-input-number> -->
			 <v-autoform v-model="bomHeaders"></v-autoform>
        </v-action-dialog>
	</v-container>
</template>
...
<script>
	module.exports = {
		...
		data: function() {
			return {
				...
				bomHeaders: [{
					...
					filter: true,
					groupable: false,
					url: App.url + "transaction/bomheader",
		...
		methods: {
			saveFromBOM: async function(){
				try {
					var self = this
					var res = await axios.get(App.url+'transaction/bomheader/addtopr', {
						params: {
							pr_id: App.page().selected.id,
							bom_header_id: self.bomHeaders[0].data
						}
					})
