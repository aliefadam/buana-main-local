    ...
    >
      ...
      <template v-slot:menu-after-filter>
        ...
        <v-btn
          ...
          >Print NPB</v-btn
        >
        <!--<v-btn small color="primary" outlined @click="completeNPB" :disabled="selected == false || (checkSelected('approved')!=0 && Number(checkSelected('item_count')||0) > 0)">Asking Approval</v-btn>-->
      </template>
      ...
      <template v-slot:item.approval_history="props">
        ...
        <span>Approved By:</span> {{ props.item.approved3_by_name }}<br />
        <span>Approved Date:</span> {{ props.item.approved3_date }}<br />
        <span>Rejected By:</span> {{ props.item.rejected_by_name }}<br />
        <span>Rejected Date:</span> {{ props.item.rejected_date }}<br />
      </template>
      <template v-slot:item.approval_notes="props">
        <span>Approved Peminta:</span><br />{{ props.item.approval1_notes
        }}<br />
        <span>Approved Kepala Bagian:</span><br />{{ props.item.approval3_notes
        }}<br />
        <span>Reject Peminta:</span><br />{{ props.item.reject_notes }}<br />
        <span>Reject Kepala Bagian:</span><br />{{ props.item.reject2_notes
        }}<br />
        <span>Revisi Peminta:</span><br />{{ props.item.revisi_notes }}<br />
        <span>Revisi Kepala Bagian:</span><br />{{ props.item.revisi_notes
        }}<br />
      </template>
      ...
      >
        <npb-item
          ...
          :table-only="[4].includes(checkSelected('approved'))"
        ></npb-item>
      </v-action-dialog>
    </v-template>
    <v-action-dialog
...
module.exports = {
  ...
  props: {
    ...
    itemsOptions: {
      ...
      default: {
        ...
        filterType: {
          //approve2_by: App.userData.data[0].userid
        },
      },
    },
  },
  data: function () {
    return {
      ...
      status: {
        "-1": "Rejected",
        0: "New",
        1: "Asking Approval Peminta",
        2: "Asking Approval Kepala Bagian",
      ...
      headers: [
        ...
        },
        {
          ...
          page: "1",
          limit: "10",
          alias: "approved3_by_name",
        },
        {
          text: "Approved Date",
          value: "approved3_date",
          align: "start",
          sortable: true,
          filterable: false,
          divider: false,
      ...
      try {
        ...
        self.$nextTick(() => {
          setTimeout(() => {
            ...
            document.querySelectorAll("#qr_approval canvas")[0].remove();
            document.querySelectorAll("#qr_approval3 canvas")[0].remove();
          }, 250);
        });
  ...
  mounted: function () {
    ...
    if (check_user(["npb_kabag"])) {
      self.itemsOptions = {
        filter: {
          flag: 1,
          // approved: "-1, 4"
          approved3_by: App.userData.data[0].userid,
        },
        filterType: {
          // approved: 'in',
        },
    ...
    if (check_user(["administrator", "logistic_admin", "logistic_npb"])) {
      self.itemsOptions = {
        filter: {
          flag: 1,
          approved: "-2, -1, 4",
