	...
	<v-container v-observe-visibility="onVisible" class="no-padding-margin" style="padding : 0 !important; height: 100%; display: flex; flex-direction: column; margin: 0; width: 100%; max-width: 100%;">
		<v-template :table-loading.sync="loading" :show-expand="showExpand" :single-expand="singleExpand" :data="data" :items-options="itemsOptions" @update:selected-row="onSelectedRow" @update:selected-row-all="onSelectedRowAll" delete-mode="delete" :active-column="activeColumn" v-model="value" ref="template" :item-key="itemKey" :url="url" :headers="headers" :name="name" :table-only="tableOnly" :table-fixed-header="tableFixedHeader">
            ...
            <template v-slot:item.details="props">
				...
				<b>Created Date:</b> {{new Date(props.item.created_date).formatDate('D MMMM YYYY')}}<br/>
			</template>
...
<script>
	module.exports = {
			...
			itemsOptions: {
				...
				default: {
					filter: {
						approved: 1,
						flag: 1,
						peminta_id: App.userData.data[0].userid
					},
					filterType: {}
				}
			}
					...
					self.$nextTick(()=>{
						setTimeout(()=>{
							...
							var qrcode = new QRCode("qr_approval", {
								text: "https://main.buanamultiteknik.com/#/sa/info/spb_approved_"+self.selected.id,
								width: 100,
								height: 100,
								colorDark : "#000000",
								colorLight : "#ffffff",
							...
							var qrcode = new QRCode("qr_approval3", {
								text: "https://main.buanamultiteknik.com/#/sa/info/spb_approved3_"+self.selected.id,
								width: 100,
								height: 100,
								colorDark : "#000000",
								colorLight : "#ffffff",
		...
		mounted: function() {
			var self = this
			loadMultipleLibrary('https://cdnjs.cloudflare.com/ajax/libs/qrcodejs/1.0.0/qrcode.min.js')
