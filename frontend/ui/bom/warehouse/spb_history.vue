  ...
  >
    ...
    >
      ...
      <template v-slot:item.approval_history="props">
        <b>Status:</b> {{ status[props.item.approved] }}<br /><br />
        <b>Validated</b><br />
        <span>By:</span> {{ props.item.approved_by_name }}<br />
        <span>Date:</span> {{ props.item.approved_date }}<br /><br />
        <b>Approved</b><br />
        <span>By:</span> {{ props.item.approved3_by_name }}<br />
        <span>Date:</span> {{ props.item.approved3_date }}<br /><br />
        <b>Rejected</b><br />
      ...
      <template v-slot:item.approval_notes="props">
        ...
        <span>Reject Logistik:</span><br />{{ props.item.reject_notes }}<br />
        <span>Reject Kepala Bagian:</span><br />{{ props.item.reject2_notes
        }}<br />
        <span>Revisi Logistik:</span><br />{{ props.item.revisi_notes }}<br />
      ...
      <template v-slot:menu-after-filter>
        <v-btn
          ...
          >Print SPB</v-btn
        >
        <!--<v-btn color="primary" v-if="!hideApproval" outlined small @click="askApproval" :disabled="disableAskApproval"> Ask Approval</v-btn>-->
      </template>
      <template v-slot:expanded-item="props">
        <td
          ref="expanded"
...
module.exports = {
    ...
    return {
      ...
      headers: [
        ...
        },
        {
          ...
          options: {
            filter: {
              groups: ["kepala_bagian", "spb_kabag"],
            },
            filterType: {
              groups: "%",
            },
  ...
  mounted: function () {
    ...
    if (check_user(["administrator", "logistic_admin"])) {
      self.itemsOptions = {
        filter: {
          flag: 1,
          approved: "-2, -1, 4",
