  ...
  >
    ...
    >
      ...
      <template v-slot:item.approval_status="props">
        <span>Status:</span> {{ status[props.item.approved] }}<br /><br />
      ...
      <template v-slot:item.approval_notes="props">
        <span v-if="props.item.ask_approval_notes"
          >Ask Approval:<br />{{ props.item.ask_approval_notes
          }}<br /><br /></span
        ><span v-else></span>
        <span v-if="props.item.approval_notes"
          >Approved Logistik:<br />{{ props.item.approval_notes }}</span
        ><span v-else></span>
...
module.exports = {
  ...
  methods: {
    ...
    approve: async function () {
      var self = this;
      var c = confirm("Are you sure want to approve this SPB?");
      if (!c) return true;
    ...
    reject: async function () {
      var self = this;
      var c = confirm("Are you sure want to reject this SPB?");
      if (!c) return true;
      if (self.selected.approved == 1) {
        var r = await axios.put(App.url + self.url, {
          approved: -1,
          pk: self.selected.id,
          reject2_notes: self.note,
          is_bom: self.selected.is_bom,
        });
        if (!r.data.status) App.errorMsg(r.data);
        else {
