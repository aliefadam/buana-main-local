    ...
    >
      ...
      <!-- 
			  <template v-slot:expanded-item="props">
			   ...
			   -->
      <template v-slot:item.pengembalian_details="props">
        <span>SPB No:</span> {{ props.item.pengembalian_no }}<br />
      ...
      <template v-slot:item.reference="props">
        <span>PO No:</span> {{ props.item.po_no }}<br />
        <span>PR No:</span> {{ props.item.pr_no }}<br />
      </template>
    ...
    >
      ...
      >
        <v-row
          no-gutters
          style="flex: 1"
          v-if="selected.approved <= 0 && selected.po_id != 0"
        >
          <v-cell cols="12" style="width: 100%">
            <pengembalian-item
              @after-insert-stock="loadPengembalianStock"
              :key="selected.id"
              :sel="processData"
              :data="dataid"
              ref="pengembalianAvailable"
            ></pengembalian-item>
          </v-cell>
        </v-row>
        <v-row no-gutters style="flex: 1">
          <v-cell cols="12" style="width: 100%">
            <stock
              ...
              @after-delete="loadPengembalianAvailable"
              :key="selected.id"
              :data="stockData"
              ref="pengembalianStock"
            ></stock>
          </v-cell>
        </v-row>
      </v-container>
...
module.exports = {
  ...
  components: {
    ...
    "pengembalian-item": "url:ui/bom/warehouse/pengembalian_item.vue",
    stock: "url:ui/bom/warehouse/pengembalian_stock.vue",
    ...
    return {
      ...
      headers: [
        ...
        },
        {
          ...
          data_value: [
            { text: "New", value: 0 },
            { text: "Asking for Approval Peminta", value: 1 },
            { text: "Asking for Approval Kepala Bagian", value: 2 },
          ],
        },
        {
          text: "Approved By",
  ...
  methods: {
    ...
    loadPengembalianStock: function () {
      var self = this;
      self.$refs.pengembalianStock.$refs.template.getItems();
    },
    loadPengembalianAvailable: function () {
      var self = this;
      self.$refs.pengembalianAvailable.$refs.template.getItems();
    },
    ...
    openPrint: async function () {
      try {
        var self = this;
        var res = await axios.get(App.url + "warehouse/pengembalianitem", {
          params: {
            filter: {
              pengembalian_id: self.selected.id,
            },
            sortBy: ["item_id"],
            limit: -1,
          },
          ...
          setTimeout(() => {
            ...
            if (self.selected.approved >= 2)
              var qrcode = new QRCode("qr_approval", {
                text:
                  "https://main.buanamultiteknik.com/#/sa/info/pengembalian_approved_" +
                  self.selected.id,
                width: 100,
                height: 100,
                colorDark: "#000000",
            ...
            if (self.selected.approved >= 4)
              var qrcode = new QRCode("qr_approval3", {
                text:
                  "https://main.buanamultiteknik.com/#/sa/info/pengembalian_approved3_" +
                  self.selected.id,
                width: 100,
                height: 100,
                colorDark: "#000000",
