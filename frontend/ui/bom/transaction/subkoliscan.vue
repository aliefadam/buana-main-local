<template>
    <v-container style="flex: 1; display: flex; flex-direction: column; min-width: 100%;" v-if="ready">
        ...
        <v-card style="display: flex; flex-direction: column; min-height: 100%; padding-bottom: 50px;">
            ...
            <div style="padding: 4px; font-weight: bold;">Location: {{location}}</div>
            <v-divider></v-divider>
			...
			transform: translateX(-50%);">
                ...
                <v-btn color="primary" outlined @click="srcImg=false; barcode = ''; headers[0].data = ''">Clear</v-btn>
                <v-btn color="primary" @click="save" :disabled="barcode=='' && srcImg == false">Save</v-btn>
                <v-spacer></v-spacer>
            </v-card-actions>
        </v-card>
...
<script>
    module.exports = {
                ...
                try {
                    ...
                    if (c === null) {} else {
                        ...
                        if (isNaN(n)) {
                            ...
                            var latlong = self.location
                            var res = await axios.post(App.url + 'transaction/bomheader/savesubkolipart', {
                                bom_receive_id: self.data.bom_receive_id,
                                subkoli_id: self.data.subkoli_id,
                                item_id: self.barcode.split('.')[1],
                                photo: self.srcImg
                            })
                            if (!res.data.status) throw res.data
                            self.headers[0].data = ''
        ...
        mounted: function() {
            var self = this
            self.getLocation().then(function(val) {
                self.location = val.lat + ',' + val.lon
            }).catch(function(err) {
                self.location = '-'
            })
            setNonPermanentInterval(() => {
                self.getLocation().then(function(val) {
                    self.location = val.lat + ',' + val.lon
                }).catch(function(err) {
                    self.location = '-'
                })
            }, 60000, randomId(), self.$el)
