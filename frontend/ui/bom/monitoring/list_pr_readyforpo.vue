...
<template>
	<v-container style="padding : 0 !important; height: 100%; display: flex; flex-direction: column; margin: 0; width: 100%; max-width: 100%;">
		<v-template :show-select="false"  :data="data" :items-options="itemsOptions"  @update:selected-row="onSelectedRow" v-model="value" ref="template" :item-key="itemKey" :url="url" :headers="headers" :name="name" table-only :table-fixed-header="tableFixedHeader" :hide-default-footer="true">
			 <template v-slot:title-body v-if="$refs.template">
                <div><b>PR - Ready for PO: </b> {{$refs.template.itemsTotal}}</div><br>
...
<script>
	module.exports = {
		...
		creator: '',
		components: {
            'pr-part': 'url:ui/bom/monitoring/list_pritem_readyforpo.vue',
		},
		props: {
			value: undefined,
            processData: {},
		...
		data: function() {
			return {
				name: '',
				itemKey: 'id',
				url: 'monitoring/listprreadyforpo',
				loading: false,
                dialogPart: false,
                dataId: {},
				headers: [{
					...
					"filter": true,
					"groupable": false,
                    "url": App.url + "monitoring/listprreadyforpo",
