<template>
    <v-container style="padding : 0 !important; height: 100%; display: flex; flex-direction: column; margin: 0; width: 100%; max-width: 100%;">
		<v-tabs style="flex: 0" v-model="tab">
			<v-tab :key="0" v-show="check_user(['andreas', 'administrator'])">Surat Permohonan (Approval 1)</v-tab>
			<v-tab :key="1" v-show="check_user(['rusbi', 'administrator'])">Surat Permohonan (Approval 2)</v-tab>
			...
			<v-tab :key="2" v-show="check_user(['andreas', 'rusbi', 'administrator'])">History Purchase Order</v-tab>
        ...
        <v-tabs-items v-model="tab" style="flex: 1;" class="tabs" touchless>
			<v-tab-item :value="0" :key="0" style="height: 100%;">
				<po v-if="dept1!=false || check_user(['andreas', 'administrator'])" :items-options="poItemsOptions" table-only btn-ask-draft></po>
			</v-tab-item>
			<v-tab-item :value="1" :key="1" style="height: 100%;">
				<po v-if="dept1!=false || check_user(['rusbi', 'administrator'])" :items-options="poItemsOptions2" table-only btn-approve-draft></po>
			</v-tab-item>
			...
			<v-tab-item :value="2" :key="2" style="height: 100%;">
				<po v-if="dept1!=false || check_user(['andreas', 'rusbi', 'administrator'])" :items-options="poItemsOptions3()" table-only nointeraction></po>
			</v-tab-item>
...
<script>
    module.exports = {
        ...
        components: {
            'po': 'url:ui/bom/asreference/af.vue',
