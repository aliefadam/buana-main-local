<template>
    <v-container v-observe-visibility="onVisible" class="no-padding-margin" style="padding : 0 !important; height: 100%; display: flex; flex-direction: column; margin: 0; width: 100%; max-width: 100%;">
        <v-template export-excel :show-select="false" :table-loading.sync="loading" :show-expand="true" :single-expand="true" :data="data" :items-options="itemsOptions" @update:selected-row="onSelectedRow" @update:selected-row-all="onSelectedRowAll" delete-mode="delete" :active-column="activeColumn" v-model="value" ref="template" :item-key="itemKey" :url="url" :headers="headers" :name="name" :table-only="tableOnly" :table-fixed-header="tableFixedHeader">
           ...
           <template v-slot:expanded-item="props">
                <td ref="expanded" :colspan="props.headers.length" style="height: auto;" :key="props.item[itemKey2]">
                    <purchase-item-group :table-fixed-header="false" :item-height-as-min-height="true" :table-fill="false" is-dashboard table-only :key="props.item[itemKey2]" :sel="{
            ...
            <template v-slot:item.total_price_per_item="props">
				{{Number(props.item.total_item_price).format(2,3)}}
			</template>
        </v-template>
    </v-container>
</template>
    ...
    module.exports = {
            ...
            return {
                ...
                headers: [/* {
                    ...
                    },
                    {
                        ...
                        "type": "list",
                        "filter": true,
                        "data_value":["New", "Asking for Approval 1", "Asking for Approval 2", "Approval 1 Approved", "Approval 2 Approved"]
                    }, {
                        "text": "RFQ No",
                        "value": "rfq_id",
                        "align": "start",
        ...
        methods: {
			...
			openReport: function(){
				...
				window.open('https://decafet.com/api/report?type=pdf&file=po&filename='+name+'&engine=easytemplate&po_id='+self.selected.po_id)
			},
			openReport2: function(){
				var self = this
				var name = self.selected.po_no.replace(/\//g, '_').replace(/\-/g, '_')
				window.open('https://decafet.com/api/report?type=pdf&file=po2&filename='+name+'&engine=easytemplate&po_id='+self.selected.po_id)
			},
