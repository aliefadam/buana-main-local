	...
	<v-container v-observe-visibility="onVisible" class="no-padding-margin" style="padding : 0 !important; height: 100%; display: flex; flex-direction: column; margin: 0; width: 100%; max-width: 100%;">
		<v-template :table-loading.sync="loading" :show-expand="showExpand" :single-expand="singleExpand" :data="data" :items-options="itemsOptions" @update:selected-row="onSelectedRow" @update:selected-row-all="onSelectedRowAll" delete-mode="delete" :active-column="activeColumn" v-model="value" ref="template" :item-key="itemKey" :url="url" :headers="headers" :name="name" :table-only="tableOnly" :table-fixed-header="tableFixedHeader">
            ...
            <template v-slot:menu-after-filter>
				<v-btn small color="primary" outlined @click="approve">Approve</v-btn>
				<v-btn small color="primary" outlined @click="revise">Revise</v-btn>
	...
	module.exports = {
		...
		methods: {
			async approve(){
				var self = this
				try {
					var p = confirm("Approve "+self.selected.pk+"?")
					if(!p) return
					var data = await axios.put(App.url+"finance/hrga", {
						status: 6,
						pk: self.selected.pk
					})		
					if(!data.data.status) throw "Approval failed!"
					App.successMsg()
			...
			async cancel(){
				var self = this
				try {
					var p = confirm("Cancel "+self.selected.pk+"?")
					if(!p) return
					var data = await axios.put(App.url+"finance/hrga", {
						status: 7,
						pk: self.selected.pk
					})		
					if(!data.data.status) throw "Cancel HRGA failed!"
					App.successMsg()
					self.$refs.template.getItems()
				} catch (error) {
					App.errorMsg(error)
			...
			async revise(){
				var self = this
				try {
					var p = confirm("Revise "+self.selected.pk+"?")
					if(!p) return
					var data = await axios.put(App.url+"finance/hrga", {
						status: 2,
						pk: self.selected.pk
					})		
					if(!data.data.status) throw "Revise failed!"
					App.successMsg()
					self.$refs.template.getItems()
				} catch (error) {
					App.errorMsg(error)
