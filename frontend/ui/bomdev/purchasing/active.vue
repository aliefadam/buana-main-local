  ...
  >
    ...
    >
        ...
        <v-row>
          ...
          <v-col cols="3">
            <v-card
              ...
              max-width="344"
              outlined
              title="Total PO with Status Asking Approval"
            >
              <v-list-item three-line>
                <v-list-item-content>
                  <div class="text-overline mb-4" style="font-weight: bold">
      ...
      <template v-slot:title-body v-if="$refs.template">
        <b>PO Active: </b>{{ $refs.template.itemsTotal }}
      </template>
      <template v-slot:item.approved="props">
        <div style="justify-content: center; display: flex">
          <v-alert
            dense
            color="#ffcc99"
            v-if="props.item.approved == 1 || props.item.approved == 2"
            style="
              margin: 0 !important;
              padding: 4px 8px;
              width: auto;
      ...
      <template v-slot:item.po_details="props">
        ...
        <b style="color: red; font-weight: bold" v-if="props.item.complete == 1"
          >Completed</b
        >
      </template>
      <template v-slot:item.shipping="props">
        <div style="">
          <b>Payment Term:</b> {{ props.item.payment_term }}<br />
      ...
      <template v-slot:item.charge1="props">
        ...
        <b>Discount Desc:</b> {{ props.item.discount_desc }}<br />
      </template>
      ...
      <template v-slot:item.approval_history="props">
        ...
        <span>Date:</span> {{ props.item.approval_date }}<br /><br />
        <b>Approved 2</b><br />
        <span>By:</span> {{ props.item.approved2_by_name }}<br />
        <span>Date:</span> {{ props.item.approval_2_date }}<br /><br />
        <b>Rejected</b><br />
...
module.exports = {
  ...
  components: {
    "purchase-item": "url:ui/bomdev/purchasing/purchaseorderitem.vue",
    "purchase-item-group":
      "url:ui/bomdev/purchasing/purchaseorderitemgroup.vue",
    "po-rev-history": "url:ui/bom/purchasing/purchaseorder_revisi.vue",
    "po-comment": "url:ui/bom/purchasing/commentx.vue",
    ...
    return {
      ...
      headers: [
        ...
        },
        {
          ...
          data_value: [
            { text: "New", value: 0 },
            { text: "Asking for Approval 1", value: 1 },
            { text: "Asking for Approval 2", value: 2 },
          ],
        },
        {
          text: "Approval Info",
  ...
  watch: {
    ...
    dialogNote: function (val) {
      if (val == false) this.revisi_note = "";
    },
    dialogAskApprovalNote: function (val) {
      if (val == false) this.approval_note = "";
    },
  },
  computed: {
    headersObj: function () {
  ...
  methods: {
    ...
    //button preview(new) (procurement)
    ...
    async getData() {
      try {
        var self = this;
        var data = await axios.get(App.url + "bom/purchaseorder/total_po");
        ...
        self.total_new_po = data.data.data[0].po_new;
        self.total_pending_po = data.data.data[0].po_pending;
      } catch (e) {}
    },
  },
  mounted: function () {
