    ...
    >
        ...
        <v-row>
          <v-col cols="3">
            <v-card
              ...
              max-width="344"
              outlined
              title="Total PR with Status Approved"
            >
      ...
      <template v-slot:menu-after-filter>
        ...
        <v-btn
          ...
          >Preview</v-btn
        >
        <!-- <v-btn small color="success" outlined v-if="tableOnly && !history" :disabled="selected == false" @click="action='approve'; dialogNote=true" >Approve</v-btn>
				<v-btn small color="warning" outlined v-if="tableOnly && !history" :disabled="selected == false" @click="action='reject'; dialogNote=true" >Reject</v-btn>
				<v-btn small color="success" outlined v-if="!tableOnly && !history" :disabled="disableAskApproval" @click="action='askApproval'; dialogNote=true" >Ask Approval</v-btn> -->
        <!--  v-if="history" -->
      </template>
  ...
  methods: {
    ...
    async getData() {
      try {
        ...
        self.total_cancel_pr = data.data.data[0].pr_rejected;
      } catch (e) {}
    },
  },
  mounted: function () {
