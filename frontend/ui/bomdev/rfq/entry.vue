        ...
        <v-template export-excel :disable-edit-button="disableEdit" :disable-delete-button="disableEdit" :table-loading.sync="loading" :show-expand="showExpand" :single-expand="singleExpand" :data="data" :items-options="itemsOptions" @update:selected-row="onSelectedRow" @update:selected-row-all="onSelectedRowAll" delete-mode="delete" :active-column="activeColumn" v-model="value" ref="template" :item-key="itemKey" :url="url" :headers="headers" :name="name" :table-only="tableOnly" :table-fixed-header="tableFixedHeader">
			<template v-slot:item.priority_show="props">
				<div style="justify-content: center; display: flex;">
					<v-alert dense color="#ffcc99" v-if="props.item.priority=='Medium'" style="margin: 0 !important; padding: 4px 8px; width: auto; display: inline-block; font-weight: bold; font-size: .875rem;" ti>
						{{props.item.priority}}
					</v-alert>
            ...
            <template v-slot:after-header>
                <v-row>
                    <v-col cols="3">
                        <v-card class="mx-auto" max-width="344" outlined  title="Total RFQ with Status New">
                    ...
                    <v-col cols="3">
                        <v-card class="mx-auto" max-width="344" outlined title="Total RFQ with Status RFQ Submitted">
                            <v-list-item three-line>
                                <v-list-item-content>
                                    <div class="text-overline mb-4" style="font-weight: bold">
                                        RFQ Submitted
                    ...
                    <v-col cols="3">
                        <v-card class="mx-auto" max-width="344" outlined title="Total RFQ with Status Clarification Needed">
                            <v-list-item three-line>
                                <v-list-item-content>
                                    <div class="text-overline mb-4" style="font-weight: bold">
                    ...
                    <v-col cols="3">
                        <v-card class="mx-auto" max-width="344" outlined title="Total RFQ with Status Quotation Received">
                            <v-list-item three-line>
                                <v-list-item-content>
                                    <div class="text-overline mb-4" style="font-weight: bold">
                                        Quotation Received
                    ...
                    <v-col cols="3">
                        <v-card class="mx-auto" max-width="344" outlined title="Total RFQ with Status Pending">
                            <v-list-item three-line>
                                <v-list-item-content>
                                    <div class="text-overline mb-4" style="font-weight: bold">
                                        Pending
            ...
            <!-- <template v-slot:title-body>
                <b>RFQ Active: </b> {{$refs.template.itemsTotal}}
            </template> -->
            ...
            <template v-slot:item.status="props">
				<div style="justify-content: center; display: flex;">
					<v-alert dense color="#ffcc99" v-if="statusRfq(props.item.status, props.item.ragic_status, props.item.rfq_group)=='Received'"  style="margin: 0 !important; padding: 4px 8px; width: auto; display: inline-block; font-weight: bold; font-size: .875rem;">
						Received
					</v-alert>
					<v-alert dense color="#f88686" v-if="statusRfq(props.item.status, props.item.ragic_status, props.item.rfq_group)=='Clarification'" style="margin: 0 !important; padding: 4px 8px; width: auto; display: inline-block; font-weight: bold; font-size: .875rem;">
						Clarification
					</v-alert>
					<v-alert dense color="#f5e699" v-if="statusRfq(props.item.status, props.item.ragic_status, props.item.rfq_group)=='New'"  style="margin: 0 !important; padding: 4px 8px; width: auto; display: inline-block; font-weight: bold; font-size: .875rem;">
						New
					</v-alert>
				</div>
            </template>
    ...
    module.exports = {
        ...
        props: {
            ...
            itemsOptions: {
                ...
                default: {
                    filter: {
                        status_s: "'New RFQ', 'RFQ Submitted', 'Clarification', 'Quotation Received', 'Pending'",
                        ragic_status:"'New', 'RFQ', 'Clarification RFQ', 'Clarification', 'Offer Sent to BMT', 'Quotation Submitted'"
                    },
                    filterType: {
                        status_s: 'in',
                        ragic_status: 'in'
        ...
        data: function() {
            return {
                ...
                headers: [{
                    ...
                    "groupable": false,
                    "default": "",
                   "data_value": ["New RFQ", "RFQ Submitted", "Clarification", "Quotation Received", "Pending"],
                }, {
                    "text": "Status Ragic",
                    "value": "ragic_status",
                    "align": "start",
        ...
        methods: {
            statusRfq: function(f, g, h){
				...
				if(h=='Local' && f == 'New RFQ' || f=='RFQ Submitted'){
					return 'New'
				}
				if(h=='Overseas Germany' && g == 'New' || g=='RFQ'){
					return 'New'
				}
                if(h=='Overseas Beijing' && g == 'New'){
					return 'New'
				}
                if(h=='Local' && f=='Clarification'){
                    return 'Clarification'
                }
                if(h=='Overseas Germany' && g == 'Clarification RFQ'){
					return 'Clarification'
				}
                if(h=='Overseas Beijing' && g == 'Clarification'){
					return 'Clarification'
				}
                if(h=='Local' && f=='Quotation Received'){
                    return 'Received'
                }
                if(h=='Overseas Germany' && g == 'Offer Sent to BMT'){
					 return 'Received'
				}
                if(h=='Overseas Beijing' && g == 'Quotation Submitted'){
					 return 'Received'
				}
			...
			async getData(){
                try {
                    ...
                    self.total_rfq = parseInt(data.data.data[0].new_rfq)+parseInt(data.data.data[0].rfq_clarification)+parseInt(data.data.data[0].rfq_quotation)
					self.total_new_rfq = data.data.data[0].new_rfq
