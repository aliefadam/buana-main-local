    ...
    module.exports = {
        ...
        methods: {
			...
			addScan(json){
				...
				try {
					...
					if(json.subkoli_id != null && json.item_id != null && json.koli_id != null){
						var check = lib.queryAll("parts", {
							query: {
								subkoli_id: json.subkoli_id,
					...
					if(count(columns) > 0 && check.length > 0){
						whereQ = {};
                        ...
                        lib.update("parts", whereQ, function(row) {
                            //row.title = "Unknown";
                            for(var i = 0; i < columns.length; i++){
                                var key = columns[i]
                                row[key] = json[key]
                                row.modified_date = new Date().formatDate('YYYY-MM-DD HH:mm')
                            }
            ...
            addKoli: async function() {
                var self = this
				var lib = new localStorageDB("koliscan", localStorage);
				if( lib.isNew() ) {
					lib.createTable("parts", ["koli_id", "item_id", "bom_header_id", "qty", "subkoli_id", "photo", "photo2", "photo3", "location", "bom_receive_id", "modified_date", "user"]);
				}
                try {
        ...
        mounted: async function() {
            ...
            loadMultipleLibrary('https://unpkg.com/html5-qrcode;./library/html5-camera.js?v1;./library/localstoragedb.js').then(function(val) {
